...
Model::Model(const string & path, bool gamma) : mGammaCorrection(gamma) {
    this->loadModel(path);
}

void Model::render(Program * program) {
    for (GLuint i = 0; i < mMeshes.size(); i++)
        this->mMeshes[i].render(program);
}
...
void Model::loadModel(string path) {
    ...
    // Retrieve the directory path of the filename
...
Mesh Model::processMesh(aiMesh * mesh, const aiScene * scene) {
    ...
    if (mesh->mMaterialIndex >= 0) {
        ...
        // 3. Normal maps
        std::vector<MeshTexture> normalMaps = this->loadMaterialTextures(material, aiTextureType_HEIGHT, "texture_normal");
        textures.insert(textures.end(), normalMaps.begin(), normalMaps.end());
        // 4. Height maps
        std::vector<MeshTexture> heightMaps = this->loadMaterialTextures(material, aiTextureType_AMBIENT, "texture_height");
...
vector<MeshTexture> Model::loadMaterialTextures(aiMaterial * mat, aiTextureType type, string typeName) {
    ...
    for (GLuint i = 0; i < mat->GetTextureCount(type); i++) {
        ...
        if (!skip) {
            ...
            this->mTexturesLoaded.push_back(texture);  // Store it as texture loaded for entire model, to ensure we won't unnecesery load duplicate textures.
        }
    }
    return textures;
}

GLuint Model::textureFromFile(const char * path, string directory, bool gamma) {
